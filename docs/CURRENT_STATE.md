...
### 미이식 화면
- [~] **어드민 `/admin`** — Step 1(서버 게이트 middleware) + Step 2 진행 중. 완료: 셸, 유저(`/admin/users`), 캐릭터(`/admin/characters`), 알림(`/admin/notifications`). 남음: curation/moderation/dashboard/eval. 그 전까진 기존 `public/admin.html`(Express adminPageGuard)이 운영용.

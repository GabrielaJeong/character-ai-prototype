...
### 미이식 화면
- [~] **어드민 `/admin`** — Step 1(서버 게이트 middleware) + Step 2 진행 중. 완료: 셸, 유저 관리(`/admin/users`), 캐릭터 관리(`/admin/characters`). 남음: notifications/curation/moderation/dashboard/eval. 그 전까진 기존 `public/admin.html`(Express adminPageGuard)이 운영용.

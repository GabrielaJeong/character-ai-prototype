...
async function generateNotification(version, content) {
  ...
  let raw;
  try {
    raw = await callGemini({ model: 'gemini-2.5-flash', systemInstruction: SYSTEM, history, maxTokens: 2048 });
  } catch (geminiErr) {
...
async function checkAndNotify() {
  ...
  // 1차 체크: state file (빠른 path, ephemeral filesystem이라 신뢰도 낮음)
  if (state.lastNotifiedVersion === version) return;

  // 2차 체크: DB에 이 버전 알림이 이미 있는지 — 영속 진실의 원천
  // (release-notify.json이 ephemeral이라 매 배포 리셋되어도 DB로 중복 차단)
  ...
  `).get(version);
  if (existing) {
    console.log(`[ReleaseNotify] ${version} 알림 이미 존재 (id=${existing.id}) — 건너뜀`);
    saveState(version); // state 파일도 동기화
    return;
  }
